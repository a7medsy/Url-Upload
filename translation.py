class Translation(object):

    ERROR = "<b>ERROR :</b> {}"

    START_TEXT = """**Hi** {},
**I'ᴍ Uʀʟ х Uᴘʟᴏᴀᴅᴇʀ!
ʏᴏᴜ ᴄᴀɴ ᴜᴘʟᴏᴀᴅ ʜᴛᴛᴘ/ʜᴛᴛᴘѕ ᴅɪʀᴇᴄᴛ ʟɪɴᴋ, ᴜѕɪɴɢ ᴛʜɪѕ ʙᴏᴛ!**

/help ғᴏʀ ᴍᴏʀᴇ ᴅᴇᴛᴀɪʟѕ!"""
    FORMAT_SELECTION = "ѕᴇʟᴇᴄᴛ ᴛʜᴇ ᴅᴇѕɪʀᴇᴅ ғᴏʀᴍᴀᴛ: <ᴀ ʜʀᴇғ='{}'>ғɪʟᴇ ѕɪᴢᴇ ᴍɪɢʜᴛ ʙᴇ ᴀᴘᴘʀᴏхɪᴍᴀᴛᴇ</ᴀ> \ɴɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ѕᴇᴛ ᴄᴜѕᴛᴏᴍ ᴛʜᴜᴍʙɴᴀɪʟ, ѕᴇɴᴅ ᴘʜᴏᴛᴏ ʙᴇғᴏʀᴇ ᴏʀ ǫᴜɪᴄᴋʟʏ ᴀғᴛᴇʀ ᴛᴀᴘᴘɪɴɢ ᴏɴ ᴀɴʏ ᴏғ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴѕ.\ɴʏᴏᴜ ᴄᴀɴ ᴜѕᴇ /ᴅᴇʟᴇᴛᴇᴛʜᴜᴍʙɴᴀɪʟ ᴛᴏ ᴅᴇʟᴇᴛᴇ ᴛʜᴇ ᴀᴜᴛᴏ-ɢᴇɴᴇʀᴀᴛᴇᴅ ᴛʜᴜᴍʙɴᴀɪʟ."
    SET_CUSTOM_USERNAME_PASSWORD = """ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ᴘʀᴇᴍɪᴜᴍ ᴠɪᴅᴇᴏѕ, ᴘʀᴏᴠɪᴅᴇ ɪɴ ᴛʜᴇ ғᴏʟʟᴏᴡɪɴɢ ғᴏʀᴍᴀᴛ:
ᴜʀʟ | ғɪʟᴇɴᴀᴍᴇ | ᴜѕᴇʀɴᴀᴍᴇ | ᴘᴀѕѕᴡᴏʀᴅ"""
    DOWNLOAD_START = "ðŸ“¥Dᴏᴡɴʟᴏᴀᴅɪɴɢ..."
    UPLOAD_START = "ðŸ“¤Uᴘʟᴏᴀᴅɪɴɢ..."
    RCHD_TG_API_LIMIT = "Dᴏᴡɴʟᴏᴀᴅᴇᴅ ɪɴ {} seconds.\nDetected File Size: {}\nSorry. But, I cannot upload files greater than 2GB due to Telegram API limitations."
    AFTER_SUCCESSFUL_UPLOAD_MSG = "**Uᴘʟᴏᴀᴅᴇᴅ Sᴜᴄᴄᴇѕғᴜʟʟʏ**"
    AFTER_SUCCESSFUL_UPLOAD_MSG_WITH_TS = "Dᴏᴡɴʟᴏᴀᴅᴇᴅ ɪɴ {} seconds.\nUᴘʟᴏᴀᴅᴇᴅ ɪɴ {} seconds"
    SAVED_CUSTOM_THUMB_NAIL = "ᴄᴜѕᴛᴏᴍ ᴠɪᴅᴇᴏ / ғɪʟᴇ ᴛʜᴜᴍʙɴᴀɪʟ ѕᴀᴠᴇᴅ. ᴛʜɪѕ ɪᴍᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴜѕᴇᴅ ɪɴ ᴛʜᴇ ᴠɪᴅᴇᴏ / ғɪʟᴇ."
    DEL_ETED_CUSTOM_THUMB_NAIL = "ᴄᴜѕᴛᴏᴍ ᴛʜᴜᴍʙɴᴀɪʟ ᴄʟᴇᴀʀᴇᴅ ѕᴜᴄᴄᴇѕғᴜʟʟʏ"
    CUSTOM_CAPTION_UL_FILE = ""
    NO_VOID_FORMAT_FOUND = "ERROR...\n<b>YouTubeDL</b> said: {}"
    HELP_USER = """ʜᴏᴡ ᴛᴏ ᴜѕᴇ ᴍᴇ? ғᴏʟʟᴏᴡ ᴛʜᴇѕᴇ ѕᴛᴇᴘѕ!
    
1. ѕᴇɴᴅ ᴜʀʟ (ᴇхᴀᴍᴘʟᴇ.ᴅᴏᴍᴀɪɴ/ғɪʟᴇ.ᴍᴘ4 | ɴᴇᴡ ғɪʟᴇɴᴀᴍᴇ.ᴍᴘ4).
2. ѕᴇɴᴅ ɪᴍᴀɢᴇ ᴀѕ ᴄᴜѕᴛᴏᴍ ᴛʜᴜᴍʙɴᴀɪʟ (ᴏᴘᴛɪᴏɴᴀʟ).
3. ѕᴇʟᴇᴄᴛ ᴛʜᴇ ʙᴜᴛᴛᴏɴ.
   ѕᴠɪᴅᴇᴏ - ɢɪᴠᴇ ғɪʟᴇ ᴀѕ ᴠɪᴅᴇᴏ ᴡɪᴛʜ ѕᴄʀᴇᴇɴѕʜᴏᴛѕ
   ᴅғɪʟᴇ  - ɢɪᴠᴇ ғɪʟᴇ (ᴠɪᴅᴇᴏ) ᴀѕ ғɪʟᴇ ᴡɪᴛʜ ѕᴄʀᴇᴇɴѕʜᴏᴛѕ
   ᴠɪᴅᴇᴏ  - ɢɪᴠᴇ ғɪʟᴇ ᴀѕ ᴠɪᴅᴇᴏ ᴡɪᴛʜᴏᴜᴛ ѕᴄʀᴇᴇɴѕʜᴏᴛѕ
   ғɪʟᴇ   - ɢɪᴠᴇ ғɪʟᴇ ᴡɪᴛʜᴏᴜᴛ ѕᴄʀᴇᴇɴѕʜᴏᴛѕ

ɪғ ʙᴏᴛ ᴅɪᴅɴ'ᴛ ʀᴇѕᴘᴏɴᴅ, ᴄᴏɴᴛᴀᴄᴛ @A7_SYR"""
    REPLY_TO_MEDIA_ALBUM_TO_GEN_THUMB = "Reply /generatecustomthumbnail to a media album, to generate custom thumbail"
    ERR_ONLY_TWO_MEDIA_IN_ALBUM = """Media Album should contain only two photos. Please re-send the media album, and then try again, or send only two photos in an album."
You can use /rename command after receiving file to rename it with custom thumbnail support.
"""
    CANCEL_STR = "Pʀᴏᴄᴇѕѕ Cᴀɴᴄᴇʟʟᴇᴅ"
    ZIP_UPLOADED_STR = "Uᴘʟᴏᴀᴅᴇᴅ {} files in {} seconds"
    SLOW_URL_DECED = "Gosh that seems to be a very slow URL. Since you were screwing my home, I am in no mood to download this file. Meanwhile, why don't you try this:==> https://shrtz.me/PtsVnf6 and get me a fast URL so that I can upload to Telegram, without me slowing down for other users."

    ERROR_YTDLP = "please report this issue on https://yt-dl.org/bug . Make sure you are using the latest version; see  https://yt-dl.org/update  on how to update. Be sure to call youtube-dl with the --verbose flag and include its complete output."
